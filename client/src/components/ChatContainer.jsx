...
import { ChatContext } from '../../context/ChrtContex'
                ...
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                        {msg.image ? (
                            ...
                            // <p className={`max-w-50 md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
                            <p
                                className={`px-4 py-2 max-w-[70%] text-sm rounded-lg mb-6 text-white
                                ${msg.senderId === authUser._id ? 'bg-violet-500 ml-auto' : 'bg-gray-700 mr-auto'}`}
                            >
                                {msg.text || " "}
                            </p>
                        )}
